...
ap.add_argument("--name", default='', help = 'name of employee')
...
while faces < max_faces:
    ...
    if len(bboxes) != 0:
        ...
        if args['name'] != '':
            if not(os.path.exists(os.path.join(dataset_path,args['name']))):
                    os.makedirs(args["output"])
            #cv2.imwrite(os.path.join(os.path.join(dataset_path,args['name']), "{}.jpg".format(faces + 1)), nimg)
